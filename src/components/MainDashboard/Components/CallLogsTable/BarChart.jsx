...
const BarChart = ({ 
  ...
  const scaleData = useMemo(() => {
    ...
    const finalMaxScaleValue = Math.max(maxScaleValue, Math.ceil(maxValue * 1.2));
    
    const scaleSteps = Math.min(8, Math.max(5, Math.ceil(finalMaxScaleValue / 50)));
  ...
  const createExportChart = () => {
    ...
    chartData.forEach((item, index) => {
      ...
      barGroup.style.borderBottom = index < chartData.length - 1 ? '1px solid #f3f4f6' : 'none';
      ...
      barWrapper.style.backgroundColor = (item.total || 0) === 0 ? '#f9fafb' : '#f3f4f6';
      barWrapper.style.border = (item.total || 0) === 0 ? '1px solid #e5e7eb' : '1px solid #d1d5db';
      ...
      if ((item.total || 0) > 0) {
        ...
        if (item.connected > 0) {
          ...
          connectedBar.style.left = `${getPercentage(item.outgoing) + getPercentage(item.incoming)}%`;
        ...
        if (item.declined > 0) {
          ...
          declinedBar.style.left = `${getPercentage(item.outgoing) + getPercentage(item.incoming) + getPercentage(item.connected)}%`;
      ...
      const metricsData = [
        { value: item.outgoing, color: getColorForMetric('outgoing'), label: 'Out' },
        { value: item.incoming, color: getColorForMetric('incoming'), label: 'In' },
        { value: item.connected, color: getColorForMetric('connected'), label: 'Conn' },
        { value: item.declined, color: getColorForMetric('declined'), label: 'Dec' }
      ];
  ...
  const exportAsPNG = async () => {
    try {
      ...
      link.href = dataURL;
      link.download = `dealer-calls-chart-${new Date().toISOString().split('T')[0]}.png`;
  ...
  const exportAsPDF = async () => {
    try {
      ...
      pdf.save(`dealer-calls-chart-${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (error) {
  ...
  const exportAsCSV = () => {
    try {
      const headers = ['Dealer Name', 'Total Calls', 'Outgoing', 'Incoming', 'Connected', 'Declined'];
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `dealer-calls-data-${new Date().toISOString().split('T')[0]}.csv`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bar-chart-container bg-white rounded-lg border border-gray-200" style={{ height }}>
        <div className="flex flex-col items-center justify-center h-full text-gray-500">
          ...
          <h3 className="text-sm font-bold text-gray-600 mb-1">No Data Available</h3>
  ...
  return (
    <div ref={chartRef} className="bar-chart-container bg-white rounded-lg border border-gray-200" style={{ height }}>
      {/* Header */}
      ...
      {/* Chart Content */}
      <div ref={chartContentRef} className="chart-content p-3" style={{ height: `calc(${height}px - 140px)` }}>
        <div className="space-y-0 max-h-full overflow-y-auto">
          {chartData.map((item, index) => {
            ...
            return (
              <div key={item.id || index} className="border-b border-gray-100 last:border-b-0">
                {/* Dealer Row - Clickable */}
                <div 
                  className={`dealer-bar-group transition-all duration-150 cursor-pointer ${
                    isSelected ? 'bg-blue-50 p-1 border border-blue-200' : 
                    isHovered ? 'bg-gray-50 p-1' : 'py-1'
                  }`}
                ...
                >
                  <div className="flex items-center gap-2">
                    {/* Dealer Name */}
                    <div className="w-40">
                      <span className={`text-sm font-medium text-[#222fb9] ${
                    ...
                    <div className="flex-1">
                      <div className={`relative w-full h-3 rounded overflow-hidden border ${
                        hasZeroValue ? 'border-gray-200 bg-gray-50' : 'border-gray-300 bg-gray-100'
                            ...
                            {item.connected > 0 && (
                              <div 
                                ...
                                style={{ 
                                  ...
                                  left: `${getPercentage(item.outgoing) + getPercentage(item.incoming)}%`,
                                  opacity: isSelected ? 1 : 0.9
                                }}
                              ></div>
                            )}
                            ...
                            {item.declined > 0 && (
                              <div 
                                ...
                                style={{ 
                                  ...
                                  left: `${getPercentage(item.outgoing) + getPercentage(item.incoming) + getPercentage(item.connected)}%`,
                                  opacity: isSelected ? 1 : 0.9
                                }}
                              ></div>
                            )}
                      ...
                      <div className="flex justify-between mt-0.5">
                        <div className="flex items-center gap-1 text-xs">
                          <div 
                            ...
                            style={{ 
                              color: isSelected ? getColorForMetric('outgoing') : '#6b7280',
                          ...
                          <div 
                            className="flex items-center gap-0.5"
                            style={{ 
                              color: isSelected ? getColorForMetric('incoming') : '#6b7280',
                              fontWeight: isSelected ? '600' : '400'
                            }}
                          >
                            <div 
                          ...
                          <div 
                            className="flex items-center gap-0.5"
                            style={{ 
                              color: isSelected ? getColorForMetric('connected') : '#6b7280',
                              fontWeight: isSelected ? '600' : '400'
                            }}
                          >
                            <div 
                          ...
                          <div 
                            className="flex items-center gap-0.5"
                            style={{ 
                              color: isSelected ? getColorForMetric('declined') : '#6b7280',
                              fontWeight: isSelected ? '600' : '400'
                            }}
                          >
                            <div 
                ...
                {isSelected && (
                  <div className="mt-1 p-2 bg-white border border-blue-200 rounded shadow-sm">
                    <div className="flex justify-between items-center mb-2">
                      <h4 className="text-xs font-bold text-gray-900">Call Details</h4>
                      <button 
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="space-y-1">
                        ...
                        <div className="flex justify-between items-center p-1 rounded" style={{ backgroundColor: `${getColorForMetric('outgoing')}15` }}>
                          <span style={{ color: getColorForMetric('outgoing') }}>Outgoing:</span>
                          <span className="font-medium">{item.outgoing || 0}</span>
                        </div>
                      </div>
                      
                      <div className="space-y-1">
                        <div className="flex justify-between items-center p-1 rounded" style={{ backgroundColor: `${getColorForMetric('incoming')}15` }}>
                          <span style={{ color: getColorForMetric('incoming') }}>Incoming:</span>
                          <span className="font-medium">{item.incoming || 0}</span>
                        </div>
                        <div className="flex justify-between items-center p-1 rounded" style={{ backgroundColor: `${getColorForMetric('connected')}15` }}>
                          <span style={{ color: getColorForMetric('connected') }}>Connected:</span>
                          <span className="font-medium">{item.connected || 0}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 pt-1 border-t border-gray-200">
                      <div className="flex justify-between text-xs">
                        <div className="text-center">
                          ...
                          <div className="font-bold">{item.total > 0 ? Math.round(((item.connected || 0) / item.total) * 100) : 0}%</div>
                        </div>
                        <div className="text-center">
                          <div className="font-medium text-blue-700">Response</div>
                          <div>{item.total > 0 ? Math.round(((item.connected + item.declined) / item.total) * 100) : 0}%</div>
                        </div>
                        <div className="text-center">
                          <div className="font-medium text-red-700">Decline</div>
                          <div>{item.total > 0 ? Math.round((item.declined / item.total) * 100) : 0}%</div>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <div className="border-t border-gray-200 px-3 py-2 bg-gray-50">
        <div className="flex justify-between items-center text-xs text-gray-600">
          ...
          <span>Active: {chartData.filter(item => (item.total || 0) > 0).length}</span>
          <span>Zero: {chartData.filter(item => (item.total || 0) === 0).length}</span>
        </div>
      </div>
